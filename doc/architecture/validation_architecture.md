...
┌─────────────────────────────────────────────────────────────┐
│                     ValidationService                        │
│  (Orchestrates all validation, manages cache, integrates)   │
└──────────────┬──────────────────────────────────────────────┘
               │
               ├──► SchemaRegistry
               │    (Manages schemas, inheritance)
               │
               ├──► OverlayValidator
               │    ├──► ValueValidator (tokens, types, formats)
               │    └──► Schema-based validation
               │
               ├──► TokenValidator
               │    (Theme token reference validation)
               │
               ├──► DataSourceValidator
               │    (Data source existence, paths)
               │
               ├──► ChartDataValidator (NEW)
...
└──────┬───────┘
       ...
       ▼
┌──────────────────────────────────────────┐
│ ValidationService.validateAll(overlays)  │
└──────┬───────────────────────────────────┘
       │
       ├──► For each overlay:
       │    │
       │    ├─1─► OverlayValidator.validate()
       │    │     ├──► Check base fields (id, type)
       │    │     ├──► Get schema from SchemaRegistry
       │    │     ├──► Validate required fields
       │    │     ├──► Validate properties (ValueValidator)
       │    │     │     ├──► Check for tokens → resolve
       │    │     │     ├──► Check for enhanced properties
       │    │     │     ├──► Validate type
       │    │     │     ├──► Validate format/range/enum
       │    │     │     └──► Return result
       │    │     ├──► Run custom validators
       │    │     └──► Validate references (anchors, attachments)
       │    │
       │    ├─2─► TokenValidator.validate()
       │    │     └──► Check token references exist
       │    │
       │    ├─3─► DataSourceValidator.validate()
       │    │     ├──► Check data sources exist
       │    │     └──► Validate paths (transformations, aggregations)
       │    │
       │    └─4─► ChartDataValidator.validate()
       │          └──► Check chart type vs. data format
       │
       ├──► Cache result
       ├──► Update statistics
