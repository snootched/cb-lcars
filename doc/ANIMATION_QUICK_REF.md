...
**Status:** Phase 1: ✅ Complete | Phase 1.5: ✅ Complete | Phase 2: 📋 Spec'd | **Targeting: ✅ Complete**
...
shimmer   # Subtle shine sweep
ripple    # Expanding wave effect

# Motion
slide     # Translate movement
rotate    # Rotation animation
motionpath # SVG path-based motion (placeholder)
...
Target specific parts of composed overlays:
...
overlays:
  - id: my_button
    ...
    animations:
      ...
      # Animate just the value when data changes
      - preset: glow
        trigger: on_datasource
        target: content  # or 'value'
        color: var(--lcars-blue)
...
- `texts[0]`, `texts[1]`, etc. - Text by array index
- `overlay` or `self` - Entire button (default if omitted)
- CSS selector - Any valid querySelector

**Text Targets:**
- `text` - The text element (default)
- `overlay` or `self` - The wrapper group
- CSS selector - Any valid querySelector

**Smart Defaults:**
- Buttons: Entire button animates when no `target` specified
- Text: Text element animates (not wrapper) when no `target` specified
- Other overlays: Entire overlay animates

**Multiple Targets:**
```yaml
targets: [label, content]  # Animates both elements with same animation
```
